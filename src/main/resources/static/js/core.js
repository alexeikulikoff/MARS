					...
					[
					  {
						...
						{  title : $profile.id, data : "id", className : "col-md-1", render : function( data, type, row ){
							...
							if ( $("#userRole").val() != "DOCTOR"){
								...
								stateButton + 
								'<li class="divider"></li>'+
								'<li><a href="#" onclick="explorations.prepareDrop(\'' + row.id + '\')"><i class="fa fa-dropbox"></i><span style="padding-left: 5px;">' + $button.drop + '</span></a></li>' + 
								'</ul>' + 
								'</div>' ; 
							}else{
								return '<span class="hidden">' + row.id + '</span><div class="btn-group">' + 
...
core.doAction = function( id, action, tableId ){
	$.ajax({
		  ...
		  success: function(e){
			 ...
			 var mm = parseInt(e.mailed);
			
			 if (mm == 0){
				 $("#btnSendMailBidding").removeAttr("disabled");
				 $("#btnSendMailBidding").html($button.bidding);
				 $("#btnSendMailBidding").attr("onclick", "core.sendMailBidding('" + id + "')");
			 }else{
				 $("#btnSendMailBidding").prop("disabled", true);
				 $("#btnSendMailBidding").html($label.invited + " " + convertUnixtimestamp( mm ));
			 }
...
core.sendMailBidding = function( id ){
	core.showWaitDialog();
	$.ajax({
		  type: "GET",
		  url: "sendMailBidding?id=" + id,
		  contentType : 'application/json',
		  dataType: "html",
		  success: function(e){
			  core.hideWaitDialog();
			  switch(e){
			  case "ERROR_UPDATING_MAILED_TIME" :  
				  core.showStatus($error.updatemailedtime,"error");
				  break;
			  case "ERROR_SENDING_EMAIL" :  
				  core.showStatus($error.sendemail,"error");
				  break;
			 default: 
				 var mm = parseInt(e);
				 $("#btnSendMailBidding").prop("disabled", true);
			 	 $("#btnSendMailBidding").html($label.invited + " " + convertUnixtimestamp( mm ));
				 core.showStatus($success.mailsend,"success");
			  }
