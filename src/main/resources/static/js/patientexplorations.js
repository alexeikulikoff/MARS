...
var patientexplorations = patientexplorations || {};
...
var documentHeight;
var svg;
patientexplorations.init = function(){
	
	//patientexplorations.showExploration
	...
	table =  $("#tbPatients")
          ...
          .on( 'init.dt', function () {
        	  ...
        	  console.log(row);
        	  patientexplorations.showExploration(row.id);
          })
		   .DataTable({
		 "language": {
				 ...
				 return '<div class="btn-group">' + 
          		'<button data-toggle="dropdown" class="btn btn-primary btn-xs dropdown-toggle" aria-expanded="false"><i class="fa fa-edit"></i>' +  
              		...
              		+ '<ul class="dropdown-menu pull-right">' + 
              			'<li><a href="#" onclick="patientexplorations.showExploration(\'' + row.id + '\')"><i class="fa fa-stethoscope"></i><span style="padding-left: 5px;">' + $button.showExploration + '</span></a></li>' +
              			'<li><a href="#" onclick="patientexplorations.newExploration(\'' + row.id + '\',\'' + row.surname + '\',\'' + row.firstname + '\',\'' + row.lastname + '\')"><i class="fa fa-plus-square"></i><span style="padding-left: 5px;">' + $button.addExploration + '</span></a></li>' +
              		 '</ul>' + 
              		'</div>' ; 
			 } }
			],
...
patientexplorations.saveExplorationViaNetwork = function(){
	var data = {
...
patientexplorations.hideExplorationForm = function(){
	$("#exDetails").addClass("hidden");
}
patientexplorations.hideNExp = function(){
...
patientexplorations.newExploration = function(id, fname, lname, sname){
...
function format ( d ) {
	...
	for(var i=0; i < d.conclusions.length; i++){
		...
		'<td><button class="btn  btn-primary btn-xs" onclick="patientexplorations.downloadConclusion(\'' + d.conclusions[i].id + '\')">' + 
...
patientexplorations.showExploration = function(id){

	$("#ExplTableContainer").empty();
	$("#ExplTableContainer").append('<table id="explorationTable" class="table"></table>');
	...
	$.ajax({
		...
		success: function(e){
			...
			explorTable = $('#explorationTable')
			.on('draw.dt', function(){
				core.hideWaitDialog();
				
				$("#exDetails").removeClass("hidden");
				$("#exDetailsNum").text(e.explorationnumber);
				$("#exDetailsPatient").text(e.username);
					...
					[
					  {
						...
						{  title : $profile.id, data : "id", className : "col-md-1", render : function( data, type, row ){
							...
							'<span class="caret"></span></button>'+ 
							'<ul class="dropdown-menu pull-right">' + 
							'<li><a href="#" onclick="patientexplorations.addConclusion(\'' + row.id + '\',\'' +  row.usersId + '\')"><i class="fa fa-desktop"></i><span style="padding-left: 5px;">' + $label.conclusion2 + '</span></a></li>' +
							'<li><a href="#" onclick="patientexplorations.downloadDicom(\'' + row.id + '\')"><i class="fa fa-download"></i><span style="padding-left: 5px;">' + $label.dicomArchive2 + '</span></a></li>' +
							'<li class="divider"></li>'+
							'<li><a href="#" onclick="patientexplorations.showDicom(\'' + row.id + '\')"><i class="fa fa-picture-o"></i><span style="padding-left: 5px;">' + $label.showDicom + '</span></a></li>' +
							'</ul>' + 
							'</div>' ; 
    		...
    		'<div class="row" style="margin-left: -5px">' + 
    		 '<div class="col-sm-6">' + 
    		  '<div class="input-group">'+
			   ...
			   '</span>' + 
			    '<input type="file" id="inpAddConclusion" class="form-control" style="display: none" oninput="patientexplorations.beforeAddConclusion(this.value)" />' +
			 '</div></div>' +  
			 '<div class="col-sm-4"><div class="btn-group">' + 
			'<button id="btnSaveExpl" class="btn btn-primary" stype="padding-left:15px;" onclick="patientexplorations.saveConclusion(\'' + id + '\',\'' + usersId +  '\')">' + 
				'<i class="fa fa-save"></i><span style="padding-left: 5px;">' +  $button.save + '</span></button></div></div></div></div></form>';
}

patientexplorations.beforeAddConclusion = function(value){	
	$("#uploadConclusionText").val(value.substring(value.lastIndexOf("\\") + 1, value.length));
	$("#uploadConclusionText").prop("disabled", true);
	
}
patientexplorations.saveConclusion = function(id, usersId){
    ...
    $.ajax({
            ...
            success: function( e ){
   			  ...
   			  case "SUCCESS_CONCLUSION_SAVE": 
   				     patientexplorations.showExploration( usersId );
...
patientexplorations.addConclusion = function( id, usersId ){
...
patientexplorations.showDicom = function( id ){
	...
	var dataSet = [];
	 $.ajax({
		  ...
		  success : function( e ){
			  ...
			  Image = getImage( cs );
			  console.log(Images);
			  var height = patientexplorations.initViewer();
					...
					.DataTable({
					 	...
					 	columns : [
						             { data: "image", className: "text-center", render : function( data, type, row, meta ){
						            	 return '<button class="btn btn-info  dim btn-large-dim btn-outline" type="button" onclick="patientexplorations.showSeria(' + row.seriaNum + ')"><img src="data:image/jpeg;base64,' + data + '" width="65" height="65" class="img-rounded"></img></button>' +
...
patientexplorations.downloadDicom  = function(id){
	 core.showWaitDialog();
	 $.ajax({
		  type: "GET",
...
patientexplorations.downloadConclusion = function(id){
...
patientexplorations.saveExploration = function(){
...
patientexplorations.initViewer = function(  ){
	 ...
	 return patientexplorations.updateSeriesView();
}	

var index = 0;
var circles = [];
var lineIndex = 0;
patientexplorations.updateSeriesView = function(  ){	
