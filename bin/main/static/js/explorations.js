...
explorations.prepareDrop = function(id){
	
	 $("#dropExplorationID").val( id );
	 $("#dropModal").addClass("show");
	
	  $("#dropModal").attr("style", "display: block; padding-right: 14px;");
...
explorations.dropExploration = function(){
	var id = $("#dropExplorationID").val();
	$.ajax({
		  type: "GET",
		  url: "dropExploration?id=" + id,
		  contentType : 'application/json',
		  dataType: "html",
		  success: function(e){
			  explorations.closeDropModal();
			  switch(e){
			  case "DROP_EXPLORATION": 
				  core.showExploration( $("#userID").val() , "admin");
				  core.showStatus($success.explDroped,"success");
				  break;
			  case "ERROR_DROP_EXPLORATION":
				  core.showStatus($error.explDroped,"error");
				  break;
			default:
				  core.showStatus($error.explDroped,"error");
			  }	  
...
explorations.updateSeriesView = function(  ){	
	 ...
	 // var k = col/row;
			...
			.on("click", function(){
					...
					var coords = d3.mouse(this);
		       		...
		       		if (index % 2 == 0){
		       			...
		       			var SIW = parseInt($("#dicomImage").attr("width"));
		       			var SIH = parseInt($("#dicomImage").attr("height"));
		       			
		       			var dx = (col / SIW) *  (circles[1].x - circles[0].x);
		       			var dy = (row / SIH) *  (circles[1].y - circles[0].y);
		       			...
		       			console.log($("#dicomImage").attr("width"));
		       			console.log($("#dicomImage").attr("height"));
	  ...
	  var Seria  = g4.append("text")
	  				...
	  				.attr("font-size", "12px")
	  				.attr("fill", "#FFFFFF");
	  var tss = parseFloat(Image.instance[0].serializedDicom.sliceThickness);
	  var SliceThicknessText = (Image.instance[0].serializedDicom.sliceThickness != null ) ? "Thickness: " + 
			  tss.toFixed(1) +" mm" : null ;
	...
	BottomRulerlength =Math.round(  PixelSpacing * (col / SIW) *  iw /( 2 * 10 ) );
...
explorations.closeDropModal = function(){
	$("#fadeIdDicom").remove();  
	$("#dropModal").removeClass("show");
	$("#dropModal").attr("style", "display: none;");
